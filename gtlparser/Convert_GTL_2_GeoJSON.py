    ...
    for item in input_json['timelineObjects']:
        try:
            if list(item.keys())[0] == 'placeVisit':
                ...
                try:
                    point = Point((point_output['centerLngE7'], point_output['centerLatE7']))
                except:
                    point = Point((point_output['longitudeE7'], point_output['latitudeE7']))
...
def parse_placeVisit_items(placeVisit):
    ...
    for aggregatedFields in subset_placeVisit:
        if aggregatedFields == 'location':
            ...
            parse_placeVisit_childVisits(subset_placeVisit.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'duration':
            parse_placeVisit_duration(subset_placeVisit.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'otherCandidateLocations':
            parse_placeVisit_location(subset_placeVisit.get(aggregatedFields), temp_allFields_dict, otherCandidateLocation=True)
        elif aggregatedFields == 'centerLatE7' or aggregatedFields == 'centerLngE7':
            temp_allFields_dict[aggregatedFields] = parse_coordinate(subset_placeVisit.get(aggregatedFields))
        else:
            temp_allFields_dict[aggregatedFields] = subset_placeVisit.get(aggregatedFields)
    return temp_allFields_dict

def parse_placeVisit_location(inputDict, temp_allFields_dict, otherCandidateLocation = False, childVisit=False):
    if otherCandidateLocation == 0:
        for subfield in inputDict:
            if subfield == 'latitudeE7' or subfield == 'longitudeE7':
                temp_allFields_dict[subfield] = parse_coordinate(inputDict.get(subfield))
            elif subfield == 'name':
                if childVisit:
                    temp_allFields_dict[subfield] = inputDict.get(subfield) + ' ' + temp_allFields_dict[subfield]
                else:
                    temp_allFields_dict[subfield] = inputDict.get(subfield)
...
def parse_placeVisit_childVisits(inputList, temp_allFields_dict):
    ...
    for aggregatedFields in childVisitDict:
        if aggregatedFields == 'location':
            parse_placeVisit_location(childVisitDict.get(aggregatedFields), temp_allFields_dict, childVisit=True)
        elif aggregatedFields == 'duration':
            parse_placeVisit_duration(childVisitDict.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'otherCandidateLocations':
            parse_placeVisit_location(childVisitDict.get(aggregatedFields), temp_allFields_dict, otherCandidateLocation=True)
        else:
...
def parse_placeVisit_duration(inputDict, temp_allFields_dict):
    for subfield in inputDict:
        temp_time_string = datetime.datetime.fromisoformat(inputDict.get(subfield)).strftime('%Y-%m-%d %H:%M:%S')
...
def parse_activitySegment_items(activitySegment):
    ...
    for aggregatedFields in subset_activitySegment:
        if aggregatedFields == 'startLocation':
            ...
            parse_activitySegment_endLocation(subset_activitySegment.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'duration':
            parse_activitySegment_duration(subset_activitySegment.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'activities':
            parse_activitySegment_activities(subset_activitySegment.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'waypointPath':
            parse_activitySegment_waypointPath(subset_activitySegment.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'simplifiedRawPath':
            parse_activitySegment_simplifiedRawPath(subset_activitySegment.get(aggregatedFields), temp_allFields_dict)
        elif aggregatedFields == 'parkingEvent':
            parse_activitySegment_parkingEvent(subset_activitySegment.get(aggregatedFields), temp_allFields_dict)
        else:
            temp_allFields_dict[aggregatedFields] = subset_activitySegment.get(aggregatedFields)
    return temp_allFields_dict

def parse_activitySegment_startLocation(inputDict, temp_allFields_dict):
    for subfield in inputDict:
        if subfield == 'latitudeE7':
            temp_allFields_dict['start_latitudeE7'] = parse_coordinate(inputDict.get(subfield))
        elif subfield == 'longitudeE7':
            temp_allFields_dict['start_longitudeE7'] = parse_coordinate(inputDict.get(subfield))
        elif subfield == 'sourceInfo':
            sourceInfo_dict = inputDict.get(subfield)
...
def parse_activitySegment_endLocation(inputDict, temp_allFields_dict):
    for subfield in inputDict:
        if subfield == 'latitudeE7':
            temp_allFields_dict['end_latitudeE7'] = parse_coordinate(inputDict.get(subfield))
        elif subfield == 'longitudeE7':
            temp_allFields_dict['end_longitudeE7'] = parse_coordinate(inputDict.get(subfield))
        elif subfield == 'sourceInfo':
            sourceInfo_dict = inputDict.get(subfield)
            parse_sourceInfo(sourceInfo_dict, temp_allFields_dict, withPrefix=True, prefix='end')
...
def parse_activitySegment_waypointPath(inputDict, temp_allFields_dict):
    for subfield in inputDict:
        if subfield == 'waypoints':
            temp_allFields_dict['waypoints'] = parse_activitySegment_waypoints(inputDict.get(subfield), temp_allFields_dict)
...
def parse_activitySegment_waypoints(inputDict, temp_allFields_dict):
    ...
    for point_dict in inputDict:
        temp_points_list.append((parse_coordinate(point_dict['lngE7']), parse_coordinate(point_dict['latE7'])))
...
def parse_activitySegment_parkingEvent(inputDict, temp_allFields_dict):
    for subfield in inputDict:
        if subfield == 'location':
            ...
            temp_allFields_dict['parkingEvent_point'] = (parse_coordinate(subDict['longitudeE7']), parse_coordinate(subDict['latitudeE7']))
            temp_allFields_dict['parkingEvent_accuracyMetres'] = subDict['accuracyMetres']
            ...
            temp_allFields_dict['parkingEvent_timestamp'] = datetime.datetime.fromisoformat(inputDict.get(subfield)).strftime('%Y-%m-%d %H:%M:%S')
