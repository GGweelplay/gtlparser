...
def parse_point_latlong(subset_visit):
    ...
    temp_lat, temp_long = temp_subset['placeLocation']['latLng'].replace('°', '').split(', ')
...
def parse_visitPoint(json_data, flag_allField=0):
    ...
    for item in json_data["semanticSegments"]:
        try:
            item_keys = list(item.keys())     # visit, timelinePath, timelineMemory, activity
            if 'visit' in item_keys:
                ...
                if flag_allField == 1:
                    point_output = {
                        ...
                        'topCadidate_probability': parse_topCadidate_probability(subset_visit)
                    }
                    point_features.append(Feature(geometry=temp_point, properties=point_output))
                else:
                    point_output = {
                        'startTime': temp_startTime,
                        'endTime': temp_endTime,
        ...
        try:
            ...
            if 'timelinePath' in item_keys:
                ...
                if len(list_points) > 1:
                    ...
                    line_features.append(Feature(geometry=temp_line, properties={'startTime': temp_startTime, 'endTime': temp_endTime}))
        except Exception as e:
            raise Exception(e)
